...
const HeaderRightContent = () => {
    ...
    useEffect(() => {
        if (user && user.id) {
            dispatch(fetchCartItems({ userId: user.id })); // ✅ FIX
        }
    }, [user, dispatch]);
    ...
    return (
        <div className="flex lg:items-center lg:flex-row flex-col gap-4">
            <Sheet open={openCartSheet} onOpenChange={setOpenCartSheet}>
                ...
                onClick={() => setOpenCartSheet(true)} variant="outline" size="icon">
                    <ShoppingCart className="w-6 h-6" />
                    <span className="absolute top-[-5px] right-[2px] font-bold text-sm ">{cartItems?.items?.length || 0}</span>
                </Button>

                {/* ✅ FIX: pass full object */}
                <CartWrapper setOpenCartSheet={setOpenCartSheet} cartItems={cartItems} />
            </Sheet>
